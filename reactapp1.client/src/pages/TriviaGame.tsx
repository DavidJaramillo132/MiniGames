...
type Result = { accepted: boolean; message?: string };
...
function TriviaGame() {
  ...
  const connectionRef = useRef<HubConnection | null>(null); const keys = useRef(new Map<number, string>()); const [state, setState] = useState<State | null>(null); const [playerIndex, setPlayerIndex] = useState<number | null>(null); const [message, setMessage] = useState('Connecting to room...');
  useEffect(() => { if (!roomId) return; const connection = new HubConnectionBuilder().withUrl(import.meta.env.VITE_SIGNALR_HUB ?? '/gameHub', { accessTokenFactory: () => useAuthStore.getState().token ?? '' }).withAutomaticReconnect().configureLogging(LogLevel.Warning).build(); connectionRef.current = connection; const onState = (next: State) => { setState(next); setMessage(next.isStarted ? 'Answer independently. Wrong answers add five seconds.' : 'Waiting for the second player.'); }; const onAssignment = (a: Assignment) => setPlayerIndex(a.symbol === 'X' ? 0 : 1); connection.on('EstadoJuegoActualizado', onState); connection.on('AsignacionJugador', onAssignment); void connection.start().then(() => connection.invoke('UnirseSala', roomId)).catch(() => setMessage('Could not connect to the room.')); return () => { connection.off('EstadoJuegoActualizado', onState); connection.off('AsignacionJugador', onAssignment); connectionRef.current = null; void connection.stop(); }; }, [roomId]);
  ...
  const answer = async (optionIndex: number) => { const connection = connectionRef.current; if (!connection || connection.state !== HubConnectionState.Connected || !roomId || playerIndex === null || !question) return; const key = keys.current.get(index) ?? crypto.randomUUID(); keys.current.set(index, key); try { const result = await connection.invoke<Result>('JugarAccion', roomId, 'answer', JSON.stringify({ questionIndex: index, optionIndex }), key); if (result.accepted) keys.current.delete(index); else { keys.current.delete(index); setMessage(result.message ?? 'Answer rejected.'); } } catch { setMessage('The answer was not confirmed. Select it again to retry safely.'); } };
